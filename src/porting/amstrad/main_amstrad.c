/* Caprice32 standalone core — talks to the firmware only through
 * gw_firmware_abi_t (sdk/src/gw_core_bridge.*). gw_core_bridge.h must
 * come last among firmware headers. */
...
#define AUDIO_BUFFER_BYTES     (AUDIO_BUFFER_LENGTH_AM * (int)sizeof(int16_t))
...
void app_main_amstrad(uint8_t load_state, uint8_t start_paused, int8_t save_slot)
{
    ...
    dtc_init();
    soundBuffer = dtc_calloc(AUDIO_BUFFER_LENGTH_AM, sizeof(int16_t));
    if (!soundBuffer)
        return;
    ...
    capmain(0, NULL);
    amstrad_set_audio_buffer((int8_t *)soundBuffer, AUDIO_BUFFER_BYTES);
