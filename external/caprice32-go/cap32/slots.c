...
#else
               /* GNW: skip track payload in file. Uncompressed uses dwTrackSize; .cdk uses per-track LZMA size (0xfe/0xff). */
               {
                  uint32_t skip_bytes = dwTrackSize;
                  if (pbGPBuffer[0x34]) {
                     skip_bytes = (uint32_t)(*(pbPtr + 0xfe) + (*(pbPtr + 0xff) << 8));
                  }
                  if (skip_bytes > 0 && fseek(pfileObject, (long)skip_bytes, SEEK_CUR) != 0) {
                     iRetCode = ERR_DSK_INVALID;
                     goto exit;
                  }
               }
