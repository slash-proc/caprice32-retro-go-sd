...
#define FDC_LZMA_SCRATCH_SIZE (32 * 1024)
#define FDC_GPBUFFER_SIZE     (16 * 1024)
static uint8_t *fdc_lzma_scratch;

static uint8_t *fdc_lzma_src(size_t compressedTrackSize)
{
   if (compressedTrackSize <= FDC_GPBUFFER_SIZE - 0x200)
      return GPBuffer + 0x200;
   if (!fdc_lzma_scratch)
      fdc_lzma_scratch = dtc_malloc(FDC_LZMA_SCRATCH_SIZE);
   if (!fdc_lzma_scratch || compressedTrackSize > FDC_LZMA_SCRATCH_SIZE)
      return NULL;
   return fdc_lzma_scratch;
}
#endif
extern t_CPC CPC;
extern t_FDC FDC;
...
#ifndef GNW_DISABLE_COMPRESSION
         drive->is_compressed = *(pbGPBuffer + 0x34);
#endif
...
#ifndef GNW_DISABLE_COMPRESSION
                  if (drive->is_compressed) {
                     uint8_t *comp_src = fdc_lzma_src(compressedTrackSize);
                     if (!comp_src || !fread(comp_src, compressedTrackSize, 1, pfileObject)) {
                        fclose(pfileObject);
                        return;
                     }
                     lzma_inflate(
...
#ifndef GNW_DISABLE_COMPRESSION
                  if (drive->is_compressed) {
                     if (fseek(pfileObject, compressedTrackSize, SEEK_CUR) != 0) {
                        fclose(pfileObject);
                        return;
                     }
                  } else
#endif
                  {
                     if (fseek(pfileObject, dwTrackSize, SEEK_CUR) != 0) {
                        fclose(pfileObject);
                        return;
                     }
                  }
...
#ifndef GNW_DISABLE_COMPRESSION
         drive->is_compressed = *(pbGPBuffer + 0x32);
#endif
